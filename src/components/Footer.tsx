...
function SocialIcon({ name, size = 16 }: { name: string; size?: number }) {
  ...
  const cls = 'fill-current';
  switch (name) {
    case 'Linkedin':
      return (
        <svg viewBox="0 0 24 24" className={cls} style={style} aria-hidden="true">
    ...
    case 'Twitter':
      return (
        <svg viewBox="0 0 24 24" className={cls} style={style} aria-hidden="true">
          <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.741l7.735-8.835L1.254 2.25H8.08l4.253 5.622zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
        </svg>
      );
    case 'Youtube':
      return (
            ...
            {socialLinks.length > 0 ? (
              <div className="flex gap-3 flex-wrap">
                {socialLinks.map((link) => (
                  ...
                  >
                    <SocialIcon name={link.icon_name} size={16} />
                  </a>
                ))}
              </div>
            ) : (
