...
export default function AdminDashboard() {
  const [stats, setStats] = useState({ projects: 0, featured: 0, contacts: 0 });
  ...
  useEffect(() => {
    Promise.all([
      ...
      setStats({ projects: p.count ?? 0, featured: f.count ?? 0, contacts: c.count ?? 0 });
      setLoading(false);
    });
  }, []);
  ...
  const statCards = [
    ...
    { label: 'Courses', value: '—', icon: BookOpen, accent: 'text-zinc-500', bg: 'bg-zinc-800/50', border: 'border-zinc-800' },
  ];
  ...
  return (
    <div className="max-w-4xl">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div className="p-6 rounded-2xl bg-zinc-900 border border-zinc-800 opacity-50 cursor-not-allowed">
          <div className="flex items-start justify-between mb-4">
            <div className="p-2.5 rounded-xl bg-zinc-800 border border-zinc-700">
              <BookOpen size={18} className="text-zinc-500" />
            </div>
            <span className="text-[10px] bg-zinc-800 text-zinc-500 px-2 py-0.5 rounded-full border border-zinc-700">Coming Soon</span>
          </div>
          <h3 className="text-white font-semibold mb-1">Manage Courses</h3>
          <p className="text-zinc-500 text-sm">Course management will be available here — add, edit and publish learning content.</p>
        </div>
      </div>
    </div>
  );
