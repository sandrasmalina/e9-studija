...
export default function AdminCourseEditPage() {
    ...
    const load = async () => {
      const [{ data: cats }, { data: course }, { data: instructorList }] = await Promise.all([
        ...
        supabase.from('courses')
          .select('title_en, title_lv, slug, short_description_en, short_description_lv, description_en, description_lv, learning_schedule_en, learning_schedule_lv, thumbnail_url, thumbnail_url_lv, promo_video_url, promo_video_type, level, language, delivery_mode, requirements, requirements_lv, what_you_learn, what_you_learn_lv, target_audience, target_audience_lv, price, discount_price, discount_ends_at, billing_type, subscription_interval, is_free, access_duration_months, starts_at, ends_at, meta_title, meta_description, meta_keywords, og_title, og_description, og_image, canonical_url, no_index, ai_summary, key_takeaways, faq_items, tags_ai_topics, expertise_level, industry, certificate_enabled, status, category_id, instructor_id')
          .eq('id', id).single(),
      ...
      if (course) {
        ...
        const nextForm: CourseForm = {
          ...
          requirements_lv: (course.requirements_lv ?? []).join('\n'),
          what_you_learn: (course.what_you_learn ?? []).join('\n'),
          what_you_learn_lv: (course.what_you_learn_lv ?? []).join('\n'),
          ...
          billing_type: course.billing_type === 'subscription' ? 'subscription' : 'one_time',
          subscription_interval: course.subscription_interval === 'year' ? 'year' : 'month',
          is_free: course.is_free ?? false,
          access_duration_months: course.access_duration_months != null ? String(course.access_duration_months) : '',
  ...
  if (loading) {
    return <div className="space-y-4">{[...Array(5)].map((_, i) => <div key={i} className="h-20 rounded-xl bg-zinc-800/50 animate-pulse" />)}</div>;
  }
  ...
  return (
    <div className="max-w-4xl">
      ...
      <div className="space-y-5">
        {/* Status — admin can set all values */}
