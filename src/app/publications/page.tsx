...
export default function PublicationsPage() {
  ...
  useEffect(() => {
    const run = async () => {
      const [{ data: publicationRows }, { data: categoryRows }, { data: linkRows }] = await Promise.all([
        supabase.from('publications').select('id,title_en,title_lv,slug,short_description_en,short_description_lv,has_lv,featured_media_url,featured_media_type,featured_image_alt,publication_date,is_featured').eq('status', 'published').order('publication_date', { ascending: false }),
  ...
  const filtered = activeCategory ? languagePublications.filter(publication => (publicationCategoryMap[publication.id] ?? []).includes(activeCategory)) : languagePublications;
            ...
            {featured && (
              <Link href={`/publications/${featured.slug}`} className="grid lg:grid-cols-[1.1fr_0.9fr] gap-0 rounded-2xl border border-white/8 bg-bg-card overflow-hidden mb-12 hover:border-accent/35 transition-colors group">
                ...
                <div className="p-8 lg:p-10 flex flex-col justify-center">
                  ...
                  <div className="flex items-center justify-between gap-4 text-sm text-neutral-500"><span className="inline-flex items-center gap-2"><CalendarDays size={15} /> {new Date(featured.publication_date).toLocaleDateString(language === 'lv' ? 'lv-LV' : 'en-GB')}</span><span className="inline-flex items-center gap-1.5 text-accent font-medium">{language === 'lv' ? 'Lasīt vairāk' : 'Read more'} <ArrowRight size={15} /></span></div>
                </div>
              </Link>
            )}
                ...
                <motion.div key={publication.id} variants={fadeUp}>
                  <Link href={`/publications/${publication.slug}`} className="block h-full rounded-2xl overflow-hidden border border-white/8 bg-bg-card hover:border-accent/30 transition-all group">
                    ...
                    <div className="p-6">
                      ...
                      <p className="text-neutral-500 text-xs mb-3">{new Date(publication.publication_date).toLocaleDateString(language === 'lv' ? 'lv-LV' : 'en-GB')}</p>
