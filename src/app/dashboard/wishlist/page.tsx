  ...
  useEffect(() => {
    (async () => {
      ...
      const { data } = await supabase
        .from('wishlists')
        .select('id, added_at, course:courses(id, title_en, thumbnail_url, slug, price, is_free, rating_avg, rating_count, instructor:profiles!courses_instructor_id_fkey(full_name))')
        .eq('user_id', user.id)
            ...
            <div key={id} className="group rounded-2xl border border-white/[0.06] bg-white/[0.02] hover:border-purple-500/30 transition-all overflow-hidden">
              ...
              <Link href={`/courses/${course.slug}`} className="block aspect-video bg-[#16122a] relative overflow-hidden">
                {course.thumbnail_url ? (
                  <img src={course.thumbnail_url} alt={course.title_en} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                ) : (
              ...
              <div className="p-4">
                <Link href={`/courses/${course.slug}`}>
                  <h3 className="text-white text-sm font-medium line-clamp-2 hover:text-purple-300 transition-colors">{course.title_en}</h3>
                </Link>
